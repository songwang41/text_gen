...
class GRUDecoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens: nn.Embedding):
        ...
        #     [DecoderLayer(config) for _ in range(config.decoder_layers)]
        # )  # type: List[DecoderLayer]
        self.gru = nn.GRU(config.d_model, config.d_model, num_layers=config.decoder_layers, batch_first=False, dropout=config.decoder_layerdrop )
        # input and output tensors are provided ( seq, batch, feature)
        # hidden states: [n_layers, feature]
        self.layernorm_embedding = LayerNorm(config.d_model) if config.normalize_embedding else nn.Identity() #None
        self.layer_norm = LayerNorm(config.d_model) if config.add_final_layer_norm else None #None
...
class BartModel(PretrainedBartModel):
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            context_vector = torch.mean(encoder_outputs[0], dim =1) # encoder_outputs[0].shape = bsz, input length, d_model
